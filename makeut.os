...
Процедура Инициализация()
	...
	УправлениеКонфигуратором.ИспользоватьВерсиюПлатформы("8.3.8");
...
мКаталогОсновнойБазы = "C:\bases\empty";
мПутьКХранилищу = "\\Orion\хранилища\Титан32";

мФайлНастроекВырезки =  ОбъединитьПути(мРабочийКаталог,"TRADE_AutoMoveMergeSettings.xml");
мФайлНастроекОбновления = ОбъединитьПути(мРабочийКаталог,"TRADE_AutoMoveUpdateSettings.xml");
мФайлОбработкиВырезки = ОбъединитьПути(мРабочийКаталог,"DeleteNoTrade.epf");
мФайлПоставки = ОбъединитьПути(мКаталогСборки,"postavka.cf");

мКаталогСборки =  ОбъединитьПути(мРабочийКаталог,"build");
...
Если ФС.ФайлСуществует(мФайлЛога) Тогда
	ВызватьИсключение "Файл "+мНоваяКонфигурацияЕРП+ " уже загружался!
...
УправлениеКонфигуратором.УстановитьКонтекст("/F""" + мКаталогОсновнойБазы + """","","");
...
УправлениеКонфигуратором.ПоместитьИзмененияОбъектовВХранилище(мПутьКХранилищу,"auto","",,"Автообновление по "+мНоваяКонфигурацияЕРП,Истина,Истина);
